...
> 给定一个输入文件D，KECI首先列举所有的跨度（达到一定长度），并将它们嵌入到特征向量中。
>
> 利用这些特征向量，KECI预测出一个初始跨度图，并应用GCN将初始关系信息整合到每个跨度表示中。
>
> 然后，KECI使用一个实体链接器来建立一个背景知识图，并应用另一个GCN来编码图的每个节点。
>
> 最后，KECI将初始跨度图和背景知识图的节点对齐，做出最终预测。
>
> 我们以端到端的方式训练KECI，不使用任何额外的实体链接监督。
