...
其中，这个 $\mathcal{C}_{\text {match }}$ 定义如下，我是没明白这里的 $\mathbb{1}$ 是什么意思？
...
&+\mathbf{p}_{\pi(i)}^{s-s t a r t}\left(s_{i}^{\text {start }}\right) \\
&+\mathbf{p}_{\pi(i)}^{s-e n d}\left(s_{i}^{e n d}\right) \\
&+\mathbf{p}_{\pi(i)}^{o-\text { start }}\left(o_{i}^{\text {start }}\right) \\
&\left.+\mathbf{p}_{\pi(i)}^{o-e n d}\left(o_{i}^{\text {end }}\right)\right]
\end{aligned}
